...
capture log close log06_noer
log using "$LOGS/06_moderation_2006_2024_noER.log", name(log06_noer) replace text
...
eststo clear
xtreg lnCE SCCD SCCD2 DEI SCCD_DEI SCCD2_DEI $BASE_CONTROLS i.year, fe vce(cluster id)
...
file open ctp_txt using "$TABLES/dei_conditional_turning_points_2006_2024_noER.txt", write replace
file write ctp_txt "DEI conditional turning points and marginal effects" _n
...
xtreg lnCE SCCD SCCD2 POLY SCCD_POLY SCCD2_POLY $BASE_CONTROLS i.year, fe vce(cluster id)
estadd local cityfe "Yes"
estadd local yearfe "Yes"
eststo mod_poly_noer

esttab mod_dei_noer mod_poly_noer using "$TABLES/table7_moderation_2006_2024_noER.rtf", replace ///
    mtitles("DEI moderator" "POLY supplementary check") ///
    b(%9.3f) se(%9.3f) star(* 0.10 ** 0.05 *** 0.01) ///
    keep(SCCD SCCD2 DEI SCCD_DEI SCCD2_DEI POLY SCCD_POLY SCCD2_POLY OPEN UR URG GI _cons) ///
    order(SCCD SCCD2 DEI SCCD_DEI SCCD2_DEI POLY SCCD_POLY SCCD2_POLY OPEN UR URG GI _cons) ///
    coeflabels(SCCD_DEI "SCCD x DEI" SCCD2_DEI "SCCD2 x DEI" SCCD_POLY "SCCD x POLY" SCCD2_POLY "SCCD2 x POLY") ///
    stats(cityfe yearfe N r2_w, labels("City fixed effects" "Year fixed effects" "N" "Within R-squared") fmt(%9s %9s 0 3)) ///
    label compress title("Table 7. Moderation and supplementary interaction checks: 2006-2024 main sample")

log close log06_noer
